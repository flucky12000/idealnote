...
import com.dycody.android.idealnote.models.listeners.OnPermissionRequestedListener;
		...
		if (ContextCompat.checkSelfPermission(activity, permission) != PackageManager.PERMISSION_GRANTED) {
			...
			if (ActivityCompat.shouldShowRequestPermissionRationale(activity, permission)) {
				Snackbar.make(messageView, rationaleDescription, Snackbar.LENGTH_INDEFINITE)
						.setAction(com.dycody.android.idealnote.R.string.ok, view -> {
		...
		RxPermissions.getInstance(activity)
				...
				.subscribe(granted -> {
					if (granted) {
						...
						String msg = activity.getString(com.dycody.android.idealnote.R.string.permission_not_granted) + ": " + permission;
