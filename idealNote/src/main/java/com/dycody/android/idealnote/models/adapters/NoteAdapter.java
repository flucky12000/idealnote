...
import com.dycody.android.idealnote.async.TextWorkerTask;
...
public class NoteAdapter extends ArrayAdapter<Note> implements Insertable {
    ...
    public NoteAdapter(Activity activity, int layout, List<Note> notes) {
        super(activity, com.dycody.android.idealnote.R.layout.note_layout_expanded, notes);
        this.mActivity = activity;
        this.notes = notes;
        this.layout = layout;

        expandedView = layout == com.dycody.android.idealnote.R.layout.note_layout_expanded;
    ...
    private void manageSelectionColor(int position, Note note, NoteViewHolder holder) {
        if (selectedItems.get(position)) {
            holder.cardLayout.setBackgroundColor(mActivity.getResources().getColor(
                    com.dycody.android.idealnote.R.color.list_bg_selected));
        } else {
            ...
            if (note.getCategory() != null && note.getCategory().getColor() != null) {
                if (colorsPref.equals("complete") || colorsPref.equals("list")) {
                    ...
                    if (holder != null) {
                        ...
                        v.findViewById(com.dycody.android.idealnote.R.id.category_marker).setBackgroundColor(Integer.parseInt(note.getCategory().getColor()));
                    }
                }
            } else {
                v.findViewById(com.dycody.android.idealnote.R.id.category_marker).setBackgroundColor(0);
            }
        }
    }
