    ...
    public static Bitmap getBitmapFromAttachment(Context mContext, Attachment mAttachment, int width, int height) {
		...
		// Audio
            ...
            bmp = ThumbnailUtils.extractThumbnail(
                    BitmapUtils.decodeSampledBitmapFromResourceMemOpt(mContext.getResources().openRawResource(com.dycody.android.idealnote.R
		...
		if (!TextUtils.isEmpty(mimeType)) {
			...
			switch (type) {
				...
				case "video":
					...
					break;
				case "audio":
					uri = Uri.parse("android.resource://" + mContext.getPackageName() + "/" + com.dycody.android.idealnote.R.raw.play);
					break;
				default:
					int drawable = "x-vcard".equals(subtype) ? com.dycody.android.idealnote.R.raw.vcard : com.dycody.android.idealnote.R.raw.files;
					uri = Uri.parse("android.resource://" + mContext.getPackageName() + "/" + drawable);
					break;
			}
		} else {
			uri = Uri.parse("android.resource://" + mContext.getPackageName() + "/" + com.dycody.android.idealnote.R.raw.files);
		}
		return uri;
	}
}
