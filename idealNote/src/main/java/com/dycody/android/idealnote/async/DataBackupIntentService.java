...
import com.dycody.android.idealnote.utils.NotificationsHelper;
...
public class DataBackupIntentService extends IntentService implements OnAttachingFileListener {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        mNotificationsHelper = new NotificationsHelper(this)
                .createNotification(com.dycody.android.idealnote.R.drawable.ic_content_save_white_24dp, getString(com.dycody.android.idealnote.R.string.working), null)
...
//        importNotes(backupDir);
    ...
    synchronized private void importDataFromSpringpad(Intent intent) {
        ...
        try {
            importer.setZipProgressesListener(percentage -> mNotificationsHelper.setMessage(getString(com.dycody.android.idealnote.R.string.extracted) + " " + percentage + "%").show());
            ...
            new NotificationsHelper(this)
                    .createNotification(com.dycody.android.idealnote.R.drawable.ic_emoticon_sad_white_24dp,
                            getString(com.dycody.android.idealnote.R.string.import_fail) + ": " + e.getMessage(), null).setLedActive().show();
            return;
        }
        ...
        for (SpringpadElement springpadElement : importer.getNotes()) {
            ...
            if (springpadElement.getType() == null) {
                Toast.makeText(this, getString(com.dycody.android.idealnote.R.string.error), Toast.LENGTH_SHORT).show();
                continue;
            }
    ...
    private void updateImportNotification(Importer importer) {
        mNotificationsHelper.setMessage(
                importer.getNotebooksCount() + " " + getString(com.dycody.android.idealnote.R.string.categories) + " ("
                        + importedSpringpadNotebooks + " " + getString(com.dycody.android.idealnote.R.string.imported) + "), "
                        + +importer.getNotesCount() + " " + getString(com.dycody.android.idealnote.R.string.notes) + " ("
                        + importedSpringpadNotes + " " + getString(com.dycody.android.idealnote.R.string.imported) + ")").show();
    }
    ...
    private void createNotification(Intent intent, Context mContext, String title, String message, File backupDir) {
        ...
        mNotificationsHelper.createNotification(com.dycody.android.idealnote.R.drawable.ic_content_save_white_24dp, title, notifyIntent)
    ...
    private boolean exportAttachments(File backupDir) {
        ...
        for (Attachment attachment : list) {
            ...
            mNotificationsHelper.setMessage(TextHelper.capitalize(getString(com.dycody.android.idealnote.R.string.attachment)) + " " + exported++ + "/" + list.size())
                    .show();
        }
        return true;
    }
    ...
    private boolean importAttachments(File backupDir) {
        ...
        while (i.hasNext()) {
            try {
                ...
                mNotificationsHelper.setMessage(TextHelper.capitalize(getString(com.dycody.android.idealnote.R.string.attachment)) + " " + imported++ + "/" + list.size())
                        .show();
            } catch (IOException e) {
                result = false;
